...
describe('node-trie', function () {
    it('should return longest string', () => {
        const input = ['rat', 'cat', 'sheep', 'ratcat', 'catrat', 'ratratrat', 'ratcatsheep', 'hippopotamus'];
        const trie1 = new Trie();
        trie1.addAll(input);
        expect((trie1.longestWord())).equal('hippopotamus');
    });
    it('should return longest compounded string', () => {
        const input = ['rat', 'cat', 'sheep', 'ratcat', 'catrat', 'ratratrat', 'ratcatsheep', 'hippopotamus'];
        const trie1 = new Trie();
        trie1.addAll(input);
        expect((trie1.longestCompoundWord())).equal('ratcatsheep');
    });
    it('should return number of items in Trie', () => {
        const input = ['rat', 'cat', 'sheep', 'ratcat', 'catrat', 'ratratrat', 'ratcatsheep'];
        const trie1 = new Trie();
        trie1.addAll(input);
        expect((trie1.WordCount())).equal(7);
    });
    ...
    it('should remove strings without delimiter', () => {
        const input = ['abc', 'abcd', 'abcdef', 'abcdefgh', 'abcdefg', 'abcf', 'abcaad', 'ab', 'Sushim', 'sus', 'N', 'nabc',
            'afg', 'naa', 'de', 'z'];
        const trie1 = new Trie();
        trie1.addAll(input);
        ...
        expect(trie1.nearMatch('z').length).equal(1);
        trie1.remove('abc');
        expect(trie1.nearMatch('abc').length).equal(6);
    });
    it('should remove strings with string delimiter', () => {
        ...
        expect((trie3._getDelimiterType())).equal(CONSTANT.STR_MATCH);
        expect(trie3.nearMatch('this').length).equal(input.length);
        trie3.removeAll(['this', 'this is']);
        expect(trie3.nearMatch('this').length).equal(1);
    });
    it('should remove strings with count delimiter', () => {
        ...
        trie2.remove('x');
        trie2.remove();
        expect(trie2.nearMatch('he').length).equal(2);
        expect(trie2.nearMatch('ab').length).equal(2);
        expect(trie2.nearMatch('abcd').length).equal(2);
        expect(trie2.nearMatch('xyz').length).equal(0);
    });
    it('should near match all strings', () => {
        const input = ['abc', 'abcd', 'abcdef', 'abcdefgh', 'abcdefg', 'abcf', 'abcaad', 'ab', 'Sushim', 'sus', 'N', 'nabc',
            'afg', 'naa', 'de', 'z'];
        const trie1 = new Trie();
        trie1.addAll(input);
        expect(_.isEmpty(trie1.nearMatchAll())).equal(true);
        expect(_.isEmpty(trie1._getFirstLevelMap())).equal(false);
        expect((trie1._hasDelimiter())).equal(false);
        expect(trie1.nearMatchAll(['abc', 'sus']).length).equal(9);
    });
    /**
     * utility test
     */
